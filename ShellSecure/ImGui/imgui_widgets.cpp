...
bool ImGui::HyperLink(const char* label, bool underlineWhenHoveredOnly)
{
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(pos, pos + size * dpi_scale);
    ItemSize(size * dpi_scale, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, IsItemActive() || hovered ? 0.1f : 0.0f, g.IO.DeltaTime * 6.f);

    it_nameanim->second *= style.Alpha;

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding * dpi_scale);
    //RenderFrame(bb.Min, bb.Max, ImColor(29 / 255.f + it_nameanim->second, 29 / 255.f + it_nameanim->second, 29 / 255.f + it_nameanim->second, 255 / 255.f), true, style.FrameRounding * dpi_scale);
...
bool ImGui::ButtonMulticolored(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    //RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding * dpi_scale);
    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, col_upr_left, col_upr_right, col_bot_right, col_bot_left, rounding, ImDrawFlags_None);
    //RenderFrame(bb.Min, bb.Max, ImColor(29 / 255.f + it_nameanim->second, 29 / 255.f + it_nameanim->second, 29 / 255.f + it_nameanim->second, 255 / 255.f), true, style.FrameRounding * dpi_scale);
...
bool ImGui::IcoButtonEx(const char* ico, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(25 / 255.f, 27 / 255.f, 44 / 255.f, 255 / 255.f), true, 16 * dpi_scale);

    PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.34f, 0.5f));



    //RenderTextClipped(bb.Min + style.FramePadding + ImVec2(27 * dpi_scale, 0), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::IcoButton(const char* ico, const char* label, const ImVec2& size_arg)
{
    return IcoButtonEx(ico, label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 cframe_col = GetColorU32(ImVec4(44 / 255.0, 199 / 255.0, 178 / 255.0, 1.00f));
    bool hovered, held;
    ...
    if (*v)
    {
        ...
        drawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, ImColor(145, 59, 99), ImColor(94, 45, 150), ImColor(94, 45, 150), ImColor(145, 59, 99), style.FrameRounding, ImDrawFlags_None);
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderFrame(check_bb.Min, check_bb.Max, ImColor(199, 253, 80, 255), true, style.FrameRounding);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
    }
    else
    {
        //auto* drawList = ImGui::GetWindowDrawList();
        RenderFrame(check_bb.Min, check_bb.Max, ImColor(2, 4, 15, 255), true, style.FrameRounding);
    }

    bool mixed_value = (g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0;

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 1.15f));


    UISettings const ui_settings{};
    auto const accent_color{ ui_settings.GetColorValue(UIColorType::Accent) };
    //window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2.85f * dpi_scale, 2.85f * dpi_scale), check_bb.Min + ImVec2(pad, pad), ImColor(accent_color.R / 255.f, accent_color.G / 255.f, accent_color.B / 255.f, it_nameanim->second), style.FrameRounding * dpi_scale);


    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x * dpi_scale, check_bb.Min.y - 2.5f + style.FramePadding.y);
...
//        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) + ImVec2(4, 4), check_col, square_sz - pad * 3.5f);
//    }
//    else if (*v == false)
//    {
//        auto* drawList = ImGui::GetWindowDrawList();
//        drawList->AddRect(ImVec2(check_bb.Min.x + 3, check_bb.Min.y + 3), ImVec2(check_bb.Max.x - 3, check_bb.Max.y - 3), GetColorU32(ImGuiCol_FrameBgDisabled), 0.0f, 0, 1.0f * dpi_scale);
//        //RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.9019f, 0.9019f, 0.9019f, 0.50f)), true, style.FrameRounding);
//    }
//
//    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x * dpi_scale, check_bb.Min.y + style.FramePadding.y);
//    if (g.LogEnabled)
...
{
    ImVec4 mode_color = ImVec4(0.8f, 0.8f, 0.8f, 1.f);
    ImVec4 key_color = ImVec4(0.8f, 0.8f, 0.8f, 1.f);
    float alpha = 0.f;
};
...
bool ImGui::Keybind(const char* label, int* key, int* mode) {
    ...
    const ImGuiID id = window->GetID(label);
    const float size = GetWindowWidth() - 73 * dpi_scale;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size * dpi_scale, 0 * dpi_scale));
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size - 14 * dpi_scale, 0 * dpi_scale), window->DC.CursorPos + ImVec2(size * dpi_scale, 23 * dpi_scale));
    ImRect key_rect;
    ...
    key_rect = ImRect(mode_rect.Min - ImVec2(11, 0), ImVec2(mode_rect.Min.x + 70 * dpi_scale, mode_rect.Max.y));
    ...
    RenderTextClipped(key_rect.Min, key_rect.Max, buf_display, NULL, NULL, ImVec2(0.0f, 0.5f));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_usable_pos_min = bb.Min[axis] + grab_padding + grab_sz * 0.7f * dpi_scale;
    const float slider_usable_pos_max = bb.Max[axis] - grab_padding - grab_sz * 0.1f * dpi_scale;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - 40 * dpi_scale;
    ...
    if ((frame_bb.Min.x + it_filing->second.value) < grab_bb.Min.x) {
        it_filing->second.speed = (grab_bb.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 4.f;
        ...
        it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - grab_bb.Min.x)) / 4.f;
        it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 160.f;
    }

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 11 * dpi_scale), ImVec2(total_bb.Min.x + w, total_bb.Min.y + 16 * dpi_scale), ImColor(55, 55, 55, 255), 30);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 11 * dpi_scale), ImVec2(frame_bb.Min.x + it_filing->second.value + 8, total_bb.Min.y + 16 * dpi_scale), ImColor(44, 199, 178, 255), 30);

    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_filing->second.value, total_bb.Min.y + 13 * dpi_scale), 9 * dpi_scale, GetColorU32(ImGuiCol_CheckMark), 30);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);


    //RenderTextClipped(frame_bb.Min - ImVec2(w, 1.f * dpi_scale), frame_bb.Max + ImVec2(w * 8.5f, 1 * dpi_scale), value_buf, value_buf_end, NULL, ImVec2(0.2f, 0.5f));

    //if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w - 41 * dpi_scale, frame_bb.Min.y + style.FramePadding.y + 1 * dpi_scale), label);
